...
# arg #2:  ftbc_debug_a:  Current absolute acceleration in km/h/s * 1000 (i.e. m/h/s), must be positive.
...
# where time(sec) = abs(ftbc_debug_cs - ftbc_debug_ts) / (ftbc_debug_a * 0.001)

# Step 1: ftbc_reg_1 = time(tick) = abs(ftbc_debug_cs - ftbc_debug_ts) * 20 / (ftbc_debug_a * 0.001)
...
scoreboard players set @s ftbc_reg_const 20000
scoreboard players operation @s ftbc_reg_1 *= @s ftbc_reg_const
scoreboard players operation @s ftbc_reg_1 /= @s ftbc_debug_a
